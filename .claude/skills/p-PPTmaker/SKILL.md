...
脚本安装路径因用户不同而不同。执行脚本前，先用 Glob 工具搜索 `**/perfect-PPTmaker/scripts/`，取所在目录作为 `SKILL_SCRIPTS_DIR`。
...
```bash
# 使用 nano-banana2 技能生成角色设定图
# 注意：--output 参数指定完整的输出文件路径
python3 /Users/jason/Desktop/heterocat-skills/.claude/skills/nano-banana2/scripts/generate_image.py \
  "Character design sheet in [STYLE] style. Keep facial features faithfully. Show 3 expressions: neutral explaining, surprised/eureka, thinking. White background, no text." \
  --image ".claude/skills/perfect-PPTmaker/assets/character/original.jpg" \
  --output "output_slides/character/character-sheet.png"
```
...
```bash
# 使用 nano-banana2 技能生成插画
# 注意：--output 参数指定完整的输出文件路径
python3 /Users/jason/Desktop/heterocat-skills/.claude/skills/nano-banana2/scripts/generate_image.py \
  "[scene description in [STYLE] style]" \
  --image ".claude/skills/perfect-PPTmaker/assets/style-samples/style-NN-name.png" \
  --output "output_slides/illustration-NN.png"
```

垫图机制：传入风格样例图或角色设定图作为参考，强制AI生成与参照图风格一致的插画。

### Path B：全AI视觉

每张slide生成完整图片（含所有文字和布局），**输出到项目根目录**：

```bash
# 使用 nano-banana2 技能生成完整slide图片
# 注意：--output 参数指定完整的输出文件路径
python3 /Users/jason/Desktop/heterocat-skills/.claude/skills/nano-banana2/scripts/generate_image.py \
  "[完整视觉描述：布局+内容+风格+情绪]" \
  --image ".claude/skills/perfect-PPTmaker/assets/style-samples/style-NN-name.png" \
  --output "output_slides/slide-NN.png"
```
...
## Step 4: 组装 PPTX（输出到项目根目录）

**Path A（html2pptx）：**

```bash
# 注意：-o 参数使用相对路径，指向项目根目录
node $(find ~/.claude -path "*/stylization-slides/scripts/html2pptx.js" 2>/dev/null | head -1) \
  slide-01.html slide-02.html ... \
  -o ../../../../output_slides/output.pptx
```
...
```bash
# 注意：-o 参数使用相对路径，指向项目根目录
python3 $(find ~/.claude -path "*/stylization-slides/scripts/create_slides.py" 2>/dev/null | head -1) \
